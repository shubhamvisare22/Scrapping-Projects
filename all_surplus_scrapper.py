...
class SurplusScrapper:
    ...
    def __init__(self):
        ...
        self.payload = json.dumps({"categoryIds": "", "businessId": "AD", "searchText": "*", "isQAL": False, "locationId": None, "model": "", "makebrand": "", "auctionTypeId": None, "page": 1, "displayRows": 100, "sortField": "bestfit", "sortOrder": "desc", "sessionId": "497d15b1-b9c6-efb5-f960-ecefa17172f6", "requestType": "search", "responseStyle": "productsOnly", "facets": [
    ...
    def make_request(self, url):
        response = requests.request(
            "POST", url, headers=self.headers, data=self.payload)
        return response
    ...
    def MainScript(self):
        ...
        self.acoount_id_list = [i['accountId']
                                for i in response['assetSearchResults']]
        self.asset_id_list = [i['assetId']
                              for i in response['assetSearchResults']]
        ...
        for i, j in zip(self.acoount_id_list, self.asset_id_list):
            ...
            self.asset_name_list.append(
                asset_name_list.strip() if asset_name_list else '-')

            asset_auction_start = asset_response['assetAuctionStartDate']
            self.asset_auction_start_list.append(asset_auction_start.split(
                'T')[0].strip() if asset_auction_start else '-')

            asset_auction_end = asset_response['assetAuctionEndDate']
            self.asset_auction_end_list.append(asset_response['assetAuctionEndDate'].split('T')[
                                               0].strip() if asset_auction_end else '-')

            seller_name_list = asset_response['sellerContactName']
            self.seller_name_list.append(
                seller_name_list if seller_name_list else '-')

            asset_seller_email_list = asset_response['sellerContactEmail']
            self.seller_email_list.append(
                asset_seller_email_list if asset_seller_email_list else '-')

            seller_phone_list_number = asset_response['sellerContactPhone']
            self.seller_phone_list.append(
                self.seller_phone_list if seller_phone_list_number else '-')
